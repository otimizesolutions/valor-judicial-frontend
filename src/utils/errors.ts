...
function parseErrorResponse(data: any, status: number, defaultMessage: string = DEFAULT_MESSAGE) {
  if (!data) {
...
function parseAxiosError(error: AxiosError<any>, defaultMessage: string = DEFAULT_MESSAGE) {
...
export function parseActionError(error: unknown, defaultMessage: string = DEFAULT_MESSAGE): ErrorData {
  if (!error) {
    return { nonFieldError: defaultMessage }
  }
...
function getObjectError(errorValue: unknown) {
...
function applyFormSubErrors(parentPath: string | null,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  form: Form<any>, fieldErrors: Record<string, unknown>) {
...
export function applyFormErrors(error: unknown,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  form: Form<any>, defaultMessage: string = DEFAULT_MESSAGE) {
...
export function applyNonFormErrors(error: unknown, defaultMessage: string = DEFAULT_MESSAGE) {
